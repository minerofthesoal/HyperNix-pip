...
class UPS:
    ...
    def check(self, *, force: bool = False) -> ThreatStatus:
        """Sample both signals (or return cached status if the last
        check was within ``check_interval_seconds``)."""
        now = time.time()
        if (
            not force
        ...
        # New panic?  Fire the snapshot_fn exactly once on the
        # transition.
        ...
        if status.panic and not was_panic and self.snapshot_fn is not None:
            try:
                ...
                status.summary += f" (snapshot_fn raised: {exc})"

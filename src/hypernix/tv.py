...
# ---------------------------------------------------------------------------
# Log parsing — generous regex so any "step N/M loss=X" line matches.
# ---------------------------------------------------------------------------
...
def _looks_like_training_log(path: Path, *, peek_bytes: int = 16384) -> bool:
    ...
    least one ``step ... loss=...`` match.  Used by autodetect to
    ...
    def poll(self) -> list[str]:
        ...
        for raw in chunk.splitlines():
            ...
            if m:
                self.latest_match = m
                try:
                    self.losses.append(float(m.group("loss")))
                except ValueError:
                    pass
        return new_lines
...
def _render_dashboard(self_tv: TVTop, f: Frame) -> str:
    ...
    if f.recent_losses:
        ...
        if len(f.recent_losses) >= 5:
            # Simple linear extrapolation of moving average
            recent_ma = sum(f.recent_losses[-5:]) / 5.0
            prev_ma = sum(f.recent_losses[-10:-5]) / 5.0 if len(f.recent_losses) >= 10 else f.recent_losses[0]
            slope = (recent_ma - prev_ma) / 5.0
            
            future_losses = []
            cur = recent_ma
            for _ in range(15):  # Predict next 15 steps
                cur += slope
                future_losses.append(max(0.0, cur))
        ...
        if c:
            colored_rows = []
            for r in graph_rows:
                # The prediction is the last 15 elements, which translates to a proportion of the graph inner width
                if len(combined) > 15:

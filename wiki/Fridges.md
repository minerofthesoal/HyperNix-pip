...
|---|---|---|
| `old_fridge` | **Weights** (freeze, stats, offload) | `freeze`, `unfreeze`, `parameter_stats`, `offload_to_cpu`, `chill_cache` |
| `mediocre_fridge` | **Datasets** (judge training examples) | `synthesize_judge_corpus`, `collect_responses_from`, `JudgeExample` |
| `new_fridge` | **Logs → graphs** | `parse_training_log`, `plot_loss_curve`, `plot_score_distribution` |

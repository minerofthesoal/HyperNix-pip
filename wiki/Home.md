...
| [T1-API](T1-API.md) | `hypernix.t1api` — controlled HTTP gateway into HyperNix-pip (model registry, auth, routing, modules, servers, jobs, events, billing). Beta 2. |
| [Waiter-TUI](Waiter-TUI.md) | `waiter` — the official T1 API client CLI/TUI (`waiter serv`, `models`, `usage`, ...). Beta 1 CLI surface (Beta 3 adds the full curses TUI). |

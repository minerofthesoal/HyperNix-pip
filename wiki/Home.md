...
| [Quantization](Quantization.md) | GGUF pipeline, k-quants, `llama-quantize` integration, `pressure_cooker_v3` QAT with FP8/Q8/Q6/Q5.5/Q4M. |
...
- **0.70.1** — UI enhancements and subsystem integrations. **WebUI** completely rewritten with a modern glassmorphism design, vibrant gradients, and micro-animations. **`tvtop`** dashboard now boots instantly (lazy-loading fixes), features PC usage history graphs (CPU, RAM, GPU, Efficiency) and a predictive training-loss extrapolation curve. **`workshop`**'s `ASRToLLMToTTS` pipeline now supports real-time conversational streaming by yielding synthesized audio chunks sentence-by-sentence. **`instant_pot`** one-shot trainer upgraded to seamlessly integrate `PressureCookerV3`, `Abbicus` token regulation, and `ComputeFramework` for DDP/FSDP. Refactored `old_oven` and `old_fridge` to gracefully unwrap distributed modules. Fixed `PressureCookerV3` premature LR drop to zero. Unit testing coverage expanded with V3 mixed-precision (FP8, FP64, Q5.5, Q4M) benchmarks.
- **0.70.0** — Five new modules + major optimizer rewrites. `abbicus` (automatic token regulation and curriculum tuning based on model size/context/dataset complexity), `compute_framework` (hardware-agnostic multi-device training with auto DDP/ZeRO wrapping for CUDA/MPS/CPU/TPU), `pressure_cooker` V2 (quantization-aware training with fp16/bf16/fp64 mixed-precision, QAT hooks for Q8/Q6/Q5.5/Q4M, 10 upgrades including gradient checkpointing, adaptive clipping, EMA shadowing, distributed awareness, dynamic loss scaling), `pressure_cooker_v3` (ZeRO-1/2 optimizations, FP8 support, `QuantDtype` enum + `QuantConfig` dataclass), `workshop` (model frameworks for TTS/ASR/LLM/Vision with pre-built templates, nano-nano collection support, 30+ architectures). `tvtop` now a backwards-compatibility shim re-exporting from `hypernix.tv`.

...
- **0.61.4** — Interactive TUI/CLI (`hypernix-cli`) with rich-based menu system, Linux Local AI Assistant with voice control (ASR input + TTS responses), Web UI with Tailscale integration at localhost:8080, enhanced ASR/TTS pipelines (`ASRToTTS`, `ASRToLLMToTTS`), 30+ new model architectures (LiquidAI LFM2.5, MiniCPM5, Gemma 4 family, Qwen3.5 series, Phi-4, DeepSeek-V2.5, GLM-Edge/MoE, GPT-OSS, Nemotron, Llama-3.2, Mistral-Nemo, Mixtral-8x22B, Nano-Nano collection), Pressure Cooker V2 lookahead bug fix, documentation updates.

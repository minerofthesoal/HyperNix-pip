"""Tests for PressureCookerV2 and PressureCookerV2Plus optimizers."""

import pytest
import torch
import torch.nn as nn

from hypernix.pressure_cooker_v2 import (
    PressureCookerV2,
...
class TestPressureCookerV2:
    """Test suite for PressureCookerV2 optimizer."""
    
    def test_basic_initialization(self):
        """Test basic initialization with default parameters."""
        model = SimpleMLP()
        optimizer = PressureCookerV2(model.parameters())
    ...
    def test_training_step_basic(self):
        ...
        model = SimpleMLP()
        optimizer = PressureCookerV2(model.parameters(), peak_lr=1e-3)
    ...
    def test_describe(self):
        ...
        assert description["kind"] == "PressureCookerV2"
    ...
    def test_invalid_parameters(self):
        ...
        with pytest.raises(ValueError, match="peak_lr must be > 0"):
            PressureCookerV2(model.parameters(), peak_lr=-1e-3)
        
        with pytest.raises(ValueError, match="grad_accum_steps must be >= 1"):
            PressureCookerV2(model.parameters(), grad_accum_steps=0)


class TestPressureCookerV2Plus:
    """Test suite for PressureCookerV2Plus optimizer with quantization."""
